...
class EntityRepositorySpec extends AnyFlatSpec with Matchers with DockerPostgresSpec {
  ...
  /**
   * INSERT for the test `members` table. Omits the `id` column (auto-generated BIGSERIAL PK). The 16 remaining columns
   * are bound positionally via [[memberInsertWrite]]. The conflict target is `natural_key` (UNIQUE constraint) so a
   * duplicate placeholder is silently ignored.
   */
