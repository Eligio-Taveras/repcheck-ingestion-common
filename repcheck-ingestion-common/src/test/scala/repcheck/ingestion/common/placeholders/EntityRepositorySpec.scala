...
class EntityRepositorySpec extends AnyFlatSpec with Matchers with DockerPostgresSpec {
  ...
  /**
   * INSERT for the `members` table. Omits `id` (BIGSERIAL auto-generated) and `created_at`/`updated_at` (DEFAULT
   * NOW()). The 14 remaining columns are bound positionally via [[memberInsertWrite]].
   */
  ...
  private def withRepo[A](
    ...
    TransactorResource
      ...
      .use { xa =>
        ...
        // Each test isolates itself by truncating before running. CASCADE required because
        // the production schema has FK references to members (e.g. member_terms).
        sql"TRUNCATE TABLE members CASCADE".update.run.transact(xa) >> block(xa, repo)
      }
      .unsafeRunSync()
